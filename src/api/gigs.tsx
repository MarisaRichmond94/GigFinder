...
const createGig = async(gig: Gig) => {
  const response = await axios.post(`${settings.BASE_SERVER_URL}/gigs`, gig);
  if (response?.data) {
    return response.data;
  }
}
...
const updateGig = async(gig: Gig) => {
  const response = await axios.patch(`${settings.BASE_SERVER_URL}/gigs/${gig.id}`, gig);
  if (response?.data) {
    return response.data;
  }
};
